    ...
    <div class="grid p-2">
      ...
      <div class="col-12 md:col-6 xl:col-3">
        <div class="card mb-0 cursor-pointer" @click="navigateTo('/students')">
          ...
          <span class="text-500"
            >из {{ totalStudents }} зарегистрированных</span
          >
        </div>
      </div>
  ...
  setup() {
    ...
    const loadDataAndStats = async () => {
      ...
      try {
        ...
        listenersInGroups.value = listenerStore.listenerList.filter(
          (listener) => listener.group_id
        ).length;
        ...
        studentStore.studentList.forEach((student) => {
          if (student.deleted_at === null) {
            const group = groupStore.groupList.find(
            ...
            if (group) {
              ...
              if (directionId) {
                directionsMap.set(
                  directionId,
                  (directionsMap.get(directionId) || 0) + 1
                );
              }
            }
          }
        ...
        directionsMap.forEach((count, directionId) => {
          const direction = directionStore.directionList.find(
            (d) => d.dir_id === directionId
          );
          if (direction) {
