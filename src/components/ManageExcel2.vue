  ...
  export default {
    ...
    setup() {
      ...
      const columnDefs = reactive({
        value: [
                 { maxWidth: 200,field: "name1", headerName: 'Фамилия' },
                 ...
                 { maxWidth: 150,field: 'dolj', headerName: 'Должность'},
                 { field: 'deg',headerName: 'Степень, звание'},
                 { maxWidth: 150,field: 'status',headerName: 'Статус'}
        ],
      });
    ...
    methods: {
        ...
        async loadTempKIT() {
          try {
            ...
            for (let i = 0;i<this.rowData.value.length;i++) {
              fam.push(this.rowData.value[i].name1)
              name.push(this.rowData.value[i].name2)
              patr.push(this.rowData.value[i].name3)
            }
        ...
        SaveToDB() {
          ...
          for (let i = 0;i<this.rowData.value.length;i++) {
              if (i==this.rowData.value.length-1) {
                teach_fio = teach_fio + "('" + name[i] + "'," + "'" + fam[i] + "'," + "'" + patr[i] + "')"
              }
              else {
                teach_fio = teach_fio + "('" + name[i] + "'," + "'" + fam[i] + "'," + "'" + patr[i] + "'),"
              }
          ...
          console.log(teach_fio)
          UserService.saveTeachers(teach_fio)
          //this.Back()
  },
