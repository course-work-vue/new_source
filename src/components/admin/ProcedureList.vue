  ...
  <div class="container-fluid p-0 d-flex flex-column flex-1" v-if="!loading">
    ...
    <div class="row g-2">
      <div class="col-12 p-0 title-container">
        <span v-if="!spisok">Список всех процедур</span>
      </div>
    </div>
    ...
    <div class="row g-2 mb-2">
      <div class="col-4 p-0">
        <button
          ...
          type="button"
        >
          <i class="material-icons-outlined me-1">add</i>Добавить процедуру
        </button>
      </div>
    </div>
  ...
  >
    <div class="card flex flex-row">
      <div class="form flex flex-1 flex-row card__form">
        <Textarea
          v-model="procedureUser.procedure_definition"
          rows="30"
          class="flex-1"
        ></Textarea>
      </div>
...
import { useProcedureUserStore } from "@/store2/admingroup/procedureUser";
...
import ProcedureUser from "@/model/admin-group/ProcedureUser";
...
export default {
  ...
  data() {
    return {
      ...
      student: new Student(),
      role: new Role(),
      procedureUser: new ProcedureUser(),
      errors: {},
      valid: false,
      scheme: null,
      scheme2: null,
  ...
  methods: {
    ...
    ...mapActions(useProcedureUserStore, [
      "getProcedureUserList",
      "postProcedureUser",
      ,
      "putProcedureUser",
      "deleteProcedureUser",
    ...
    resetProcedureUser() {
      this.role = new Role();
      this.procedureUser = new ProcedureUser();
    },
    edit(event) {
      this.resetProcedureUser();
      this.procedureUser = event.data;
    ...
    async submit() {
      let procedureUser = { ...this.procedureUser };
      if (procedureUser.id) {
        await this.putProcedureUser(procedureUser.procedure_definition);
      } else {
        await this.postProcedureUser(procedureUser.procedure_definition);
      }
    ...
    async deleteR() {
      ...
      await this.deleteProcedureUser(procedureUser);
      this.formVisible = false;
      this.resetProcedureUser();
      this.loadProceduresData();
    },
    ...
    async loadProceduresData() {
      try {
        if (Array.isArray(this.roleList)) {
          ...
          this.rowData.value = this.procedureUserList;
        } else {
    ...
    onFilterChanged() {
      // This procedure will be called whenever filters change.
  ...
  computed: {
    ...
    ...mapState(useProcedureUserStore, [
      "procedureUserList",
      "procedureUserMap",
    ]),
