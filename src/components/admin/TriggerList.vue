...
import { useTriggerUserStore } from "@/store2/admingroup/triggerUser";
...
import TriggerUser from "@/model/admin-group/TriggerUser";
...
export default {
  ...
  methods: {
    ...
    ...mapActions(useTriggerUserStore, [
      "getTriggerUserList",
      "postTriggerUser",
      ,
      "putTriggerUser",
    ...
    resetTriggerUser() {
      this.role = new Role();
      this.triggerUser = new TriggerUser();
    },
    edit(event) {
      this.resetTriggerUser();
      this.triggerUser = event.data;
    ...
    async submit() {
      let triggerUser = { ...this.triggerUser };
      if (triggerUser.id) {
        await this.putTriggerUser(triggerUser.trigger_definition);
      } else {
        await this.postTriggerUser(triggerUser.trigger_definition);
      }
    ...
    async deleteR() {
      ...
      await this.deleteTriggerUser(triggerUser);
      this.formVisible = false;
      this.resetTriggerUser();
      this.loadTriggersData();
    },
    ...
    async loadTriggersData() {
      try {
        if (Array.isArray(this.roleList)) {
          ...
          this.rowData.value = this.triggerUserList;
        } else {
  ...
  computed: {
    ...
    ...mapState(useTriggerUserStore, ["triggerUserList", "triggerUserMap"]),
