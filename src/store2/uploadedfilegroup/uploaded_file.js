...
export const useUploaded_FileStore = defineStore('uploaded_file', {
    ...
    getters: {
        uploaded_fileMap(state) {
            return state.uploaded_fileList.reduce((map, uploaded_file) => {
                map[uploaded_file.direction_code] = uploaded_file;
                return map;
            }, {});
        },
    },
    actions: {
        ...
        async putUploaded_File(uploaded_file) {
            const response = await api.putUploaded_File(uploaded_file.direction_code, uploaded_file);
            if (response.success === true) {
                const index = this.uploaded_fileList.findIndex(s => s.direction_code === uploaded_file.direction_code);
                if (index !== -1) {
